...
# this is the harder version of the game, where the AI is "smarter"
      ...
      # finds the next best move for the virus and the best next move for the player (opponent)
         ...
         for row, col in self.dict:
            ...
            if (self.dict[row,col] == self.emptyCell):
               # recursive call at each level to see all available moves
...
# inherits from Minimax Class for the method of "AI"k
#################################### 
...
class TicTacVirusBoard(Minimax):
   ...
   def gameOver(self):
      ...
      for col in range(self.size):
         winner = []
         for row in range(self.size):
            if (self.dict[row, col] == self.virus):
      ...
      winner = []
      for col in range(self.size):
         if (self.dict[col, col] == self.virus):
      ...
      winner = []
      for col in range(self.size):
         if (self.dict[self.size - col - 1, col] == self.virus):
...
class DisplayTicTacVirusBoard(object):
   ...
   def update(self):
      ...
      # if the game is a tie, the user has "survived" the virus and receives the next object
